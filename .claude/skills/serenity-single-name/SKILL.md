...
Deliver separated `Fact`/`Inference`/`Action`, the lens or its evidence gap, current trigger, bear case, and falsifiers. Use `NFA` only for actionable `RECOMMEND_NOW` or `ENTER_ON_TRIGGER` conclusions. A finalized decision with a trigger or a falsifier worth revisiting can be registered with `"$SERENITY_PYTHON" "$SERENITY_CLI" outcomes register` and measured later with `outcomes refresh`; this is optional by design, because a decision never revisited is a research choice rather than an unfinished run. Hand only a remaining peer question to cohort research or a missing chain relationship to discovery; do not ask a downstream mode to replace the single-name decision.
