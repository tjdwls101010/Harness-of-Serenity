...
For a tiny-TAM input that physically GATES a huge deployment, its current TAM is a FLOOR not a ceiling. Price it as an option on the gated end-market — willingness-to-pay is set by the COST OF NOT HAVING IT (a $100 part that strands a $20B buildout commands orders of magnitude more than its commodity spot), not the input's price. Consensus systematically UNDER-values these by anchoring on input TAM; that gap is the asymmetry. When the bull case rests on an **adjacent compute/scaling curve** about to need a niche material (AlSiC → AI-thermal at 2000W+, InP → photonics), carry that pivot as a SEPARATE "option on the story" leg (R5) — never folded into disclosed run-rate backlog — and weigh **strategic-scarcity × MC** (% of Western/qualified supply × MC) alongside EV/Rev, since EV/Rev alone under-credits a sole-source qualified node mid-trough. A single cost-spike / pass-through trough quarter is a cycle-stage read (V2/V10), not proof the moat is replicable — don't let it downgrade chokepoint → constraint without asking whether the spiking cost IS the input the scarcity rests on.
...
- **Valuation with the LENS NAMED** — EV-multiple banding floor-first for a has-revenue physical name; for an asset-financed / pre-scale / disruptor name, compute the floor first and declare it N/A ONLY when it demonstrably fabricates an absurd verdict, then substitute a driver-based anchor DEFENDED with a real checkable number (a levered IRR from prepayment + financing terms, a contracted-customer-funded backlog $, a sanity-banded TAM); for a design-win component supplier the lens is the bottom-up content × end-volume ÷ MC, run *before* calling it priced. Naming the lens is what stops a disruptor self-declaring "the framework breaks" as a free pass out of valuation discipline.
  - **GATE — the lens must be RUN, not just named.** Before you print *priced / cheap / WATCH*, show the **arithmetic** of the lens's driver number (content×volume÷MC · $/MW levered IRR · replacement-cost-per-unit · pro-forma FCF · single-project floor), each input traced to the dossier / `key_facts`. If the call rests only on a standalone **top-down** multiple — the subject's *own* EV/Rev, PEG, or a TAM cross — the lens did NOT run: that top-down read IS the consensus lens the gap is built against, so a "priced / pass" verdict from it is *unfinished, not conservative*. Catching yourself reach for the subject's own PEG is the trigger to run the bottom-up move, not to conclude.
...
**Close comparatively** — rank the name against its alternatives even on a single-ticker ask (*"strong, but X in the same chain is more compelling / faster"*), so the power-law instinct is audible: a name is only a buy relative to the best alternative in its chain. On a countable-end-unit supplier or an inherently-comparative archetype (neocloud / commodity-capacity / margin-inflection), make this concrete: **run `serenity_pipeline.py discover SUBJECT PEER1 PEER2 …`** and show the MC-vs-forward-revenue ranking — the cross-peer ratio, not the subject's own standalone multiple, is the verdict. (The `Downsides:` block, the explicit bear case / falsifier, and the US-listed output gate are always-on invariants from CLAUDE.md surfacing here — honored at this layer, defined there.)
