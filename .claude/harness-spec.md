...
`session_health.py` is local and network-free. It checks shared symlinks, settings shape, current entry files, the presence of the mandatory SEC contact identity, and the consistency of any active pointer/manifest. The credential is checked for presence only and never echoed, because a missing `SERENITY_SEC_USER_AGENT`/`EDGAR_IDENTITY` stops every identity-pinned run at the SEC gate and is worth naming before a run opens rather than after. A healthy hook emits no stdout and exits 0. A degraded hook still exits 0 and emits only `hookSpecificOutput.additionalContext`, phrased as a factual local-state diagnostic so ordinary work can continue through transient provider or local setup issues.
