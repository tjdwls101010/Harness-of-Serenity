...
## HARNESS COMPLETE + VALIDATED (WF1→WF4 done). Validation loop concluded.
WF4 ran 4 rounds (build → validate → patch → re-validate); the harness reproduces serenity's framework + applies broader/stricter data than the source (judges' own words), with 3 committed doctrine rounds that landed the signature moves and made the verdict-gate un-buryable (CLAUDE.md N10 + sign-default fixes + the always-on evidence_discipline hook). Loop concluded: the residual fidelity gap is verdict-time LLM variance + a measurement confound (WF4 subagents don't get the hook; scores noisy run-to-run), not a fixable doctrine gap — chasing it further risks bloat/overfit. Full detail in the [[wf4-validation-finding]] memory. A future trustworthy re-measure would need point-in-time data + the hook injected into the reproduction agents.

## (historical) BUILD COMPLETE — only WF4 (validation) remained. Resume at "THEN WF4" below.
