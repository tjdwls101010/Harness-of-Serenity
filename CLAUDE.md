...
10. Before printing a valuation verdict (priced / cheap / pass / buy): the lens the archetype demanded must be **RUN, not just named** — show its driver arithmetic (content×vol÷MC · $/MW levered IRR · replacement-cost-per-unit · pro-forma FCF · supply-shock ASP · floor+option), each input traced to key_facts/dossier. A verdict resting on the subject's OWN top-down multiple (its EV/Rev, PEG, or a trailing-margin snapshot) IS the consensus read the gap is built against — *unfinished, not conservative*. And a **negative/thin trailing margin** on a name mid-capex-buildout, mid-restructuring-trough, or in a pass-through cost-spike is a STAGE read, resolved on revenue/margin *trajectory* — never the structural verdict that authors a bearish call. And a forked lens is only **HALF-run** when only the floor/discount/dilution number is shown and the upside leg (the re-rate, replacement-cost steal, FCF bridge, content-leapfrog, supply-shock ASP) is left qualitative — *unfinished, not conservative*: show the bull-leg arithmetic beside the floor and let the two legs fight (**run both legs, never *resolve bullish***). A bear-leg-only call silently inverts a power-law long into a pass — the single biggest direction-miss.
