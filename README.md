...
SEC filing evidence is requested by capability: `sec.submissions`, `sec.filings`, `sec.filing-text`, `sec.filing-section` (`named` accepts `business`, `risk_factors`, `mda`), `sec.xbrl-facts`, `sec.segments`, `sec.statement`, and `sec.eightk`. Each carries the accession, the SEC acceptance instant, and exact raw bytes, so a section read stays traceable to the filing it came from. `sec.filing-section`'s `named` values are resolved per form -- a 10-K defines all three, a 10-Q defines `risk_factors` and `mda` -- and an unsupported form/section pair returns a typed `invalid` naming what the form does define, never `not_disclosed`. `alfred-fred.macro-series` returns the one vintage in force at the cutoff; `alfred-fred.vintage-series` returns every vintage published by then and requires `observation_start`, because an unbounded revision history returns a row per observation per vintage. Read any capability's parameter contract with `evidence catalog --capability <id>`; the registry validates a request against it before constructing a provider, so a wrong shape is refused by name rather than spent on a request.
...
The private raw cache grows with use rather than being pruned: five runs produced about 58 MB, and a single SEC filing's exact bytes run 9-19 MB. There is deliberately no retention policy, because deleting a raw payload breaks the hash verification that every past decision's provenance depends on -- budget disk for it, and archive whole runs rather than trimming payloads inside one. Collected evidence is also read in bounded form by default: `evidence collect` and `evidence read` answer with the value's shape, `--value` prints the payload, and `evidence read RESULT_ID --match REGEX` returns matching spans with character offsets into the stored string, so citing a 144k-character section costs kilobytes and stays verifiable against the artifact.
